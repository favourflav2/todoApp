...
export default function EditAndAddTask() {
  ...
  const { id } = useParams();
  const data = todos.find((item) => item.createdAt === id);
  ...
  const [dateIsPastError, setDateIsPastError] = React.useState<any>("");
  const [dateIsPastText, setDateIsPastText] = React.useState("");
  ...
  function editArrayTodo() {
    if (textState.taskName && timeData && dateData && priorityState && complexityState) {
      // If we have an id ... means we are on item details page
      if (data) {
  ...
  const todoObj: Todo = {
    ...
    createdAt: data ? data?.createdAt : createNewDate,
  };
  ...
  React.useEffect(() => {
    ...
    if (dateIsPastError) {
      setDateIsPastText("This date has pasted, we recommend that you change it.");
    } else {
      setDateIsPastText("");
    }
  }, [errors, textState, priorityState, complexityState, dateData, timeData, dateIsPastError]);
    ...
    <div className="w-full flex h-auto p-10 justify-center  bg-gray-200">
      ...
      >
        ...
        <div className="flex justify-center items-center relative w-full">
          <h1 className="title text-[28px] font-medium">{data?.title ? "Edit Task" : "Add Task"}</h1>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DemoContainer components={["DatePicker"]}>
                <DatePicker
                  ...
                  disablePast
                  className={`${errors.date ? "bg-red-200" : ""} w-full`}
                  onError={(newError) => setDateIsPastError(newError)}
                  slotProps={{
                    textField: {
                      helperText: dateIsPastText,
                    },
                  }}
                />
              </DemoContainer>
          ...
          <div className="w-full h-auto flex flex-col  ">
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DemoContainer components={["TimePicker"]}>
                <TimePicker value={timeData} onChange={(newValue) => setTimeData(newValue)} className={`${errors.time ? "bg-red-200" : ""} w-full`} />
              </DemoContainer>
            </LocalizationProvider>
