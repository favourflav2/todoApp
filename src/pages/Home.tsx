  ...
  return (
    <div className="w-full min-h-screen flex p-10 justify-center items-center bg-gray-200">
      ...
      <div className="main-content  w-[80%] min-h-[600px] flex flex-col items-center   ">
        ...
        {/* Power Button */}
        {powerOnAndOff ? (
          <button className="my-8 bg-blue-400 rounded-full px-8 py-4 text-white" onClick={() => setPowerOnAndOff((item) => !item)}>
            <PowerIcon /> Power On
          </button>
        ) : (
          <button className="my-8 bg-gray-400 rounded-full px-8 py-4 text-white" onClick={() => setPowerOnAndOff((item) => !item)}>
